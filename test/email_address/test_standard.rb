...
class TestStandardAddress < Minitest::Test
  ...
  def test_parse
    a = EmailAddress.standard("(lcl)User+tag(lcr)@(dcl)example.com(dcr)")
    assert_equal "User+tag",    a.local_name
    assert_equal "(lcl)",       a.local_comment_left
    assert_equal "(lcr)",       a.local_comment_right
    assert_equal "example.com", a.domain_name
    assert_equal "(dcl)",       a.domain_comment_left
    assert_equal "(dcr)",       a.domain_comment_right
