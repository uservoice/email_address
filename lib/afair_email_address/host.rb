...
module AfairEmailAddress
  ##############################################################################
  # The AfairEmailAddress Host is found on the right-hand side of the "@" symbol.
  # It can be:
  ...
  class Host
    ...
    def find_provider # :nodoc:
      ...
      if provider != :default
        self.set_provider(provider,
          AfairEmailAddress::Config.provider(provider))
      end
    ...
    def set_provider(name, provider_config={}) # :nodoc:
      self.config = AfairEmailAddress::Config.all_settings(provider_config, @config)
      self.provider = name
    end
    ...
    def dns_enabled?
      [:mx, :a].include?(AfairEmailAddress::Config.setting(:host_validation))
    end
    ...
    # Returns an array of AfairEmailAddress::Exchanger hosts configured in DNS.
    # The array will be empty if none are configured.
    def exchangers
      #return nil if @config[:host_type] != :email || !self.dns_enabled?
      @_exchangers ||= AfairEmailAddress::Exchanger.cached(self.dns_name, @config)
    end
